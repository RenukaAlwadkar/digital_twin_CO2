...
void publishTraffic() {
  ...
  snprintf(payload, sizeof(payload),
           "{\"nodeId\":\"traffic-collector\",\"city\":\"Delhi\",\"corridor\":\"Ring Road\",\"trafficDensity\":%.1f,\"averageSpeedKph\":%.1f,\"vehicleCount\":%d,\"co2ppm\":%.1f,\"pm25\":%.1f,\"aqi\":%d,\"incident\":%s,\"timestamp\":%lu}",
           trafficDensity,
           averageSpeedKph,
           vehicleCount,
           co2ppm,
...
void ensureConnectivity() {
  // Avoid tight reconnect loops in simulation.
  ...
  if (!mqttClient.connected()) {
    String clientId = "traffic_collector_" + String((uint32_t)ESP.getEfuseMac(), HEX);
